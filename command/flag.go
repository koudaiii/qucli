...
func FlagInit(args []string) error {
	...
	flags.StringVar(&event, "event", "repo_push", "set 'evnet'.  ['repo_push', 'build_queued', 'build_start', 'build_success', 'build_failure', 'build_cancelled', 'vulnerability_found'].")
	flags.StringVar(&method, "method", "webhook", "set 'method'.  ['webhook', 'slack', 'email'].")
	flags.StringVar(&url, "url", "http://url/goes/here", "if you use 'webhook' or 'slack' method, set url. 'http://url/goes/here' or 'https://hooks.slack.com/service/{some}/{token}/{here}'.")
	...
	flags.StringVar(&ref, "ref", "", "An optional regular expression for matching the git branch or tag git ref. If left blank, the notification will fire for all builds.(refs/heads/somebranch)|(refs/tags/sometag) if you use repo_push, you can not set ref.")
